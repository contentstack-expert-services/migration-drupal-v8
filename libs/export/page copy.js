...
const { localeSwitch } = require('./localeSwitch.js');
const { localeMapper } = require('./localeMapper.js');
let localePost = [];
var finalLocale;
var localVar;
var localeCode;
...
ExtractPosts.prototype = {
 ...
 putPosts: function (postsdetails, key, countentry) {
     ...
     // let localePost = postsdetails[0]?.langcode;

     for(const localValue of Object.values(postsdetails)){
         localeCode = localeSwitch(localValue.langcode);
         // localVar = localeMapper(localValue.langcode)
         // localePost = _.union([localValue.langcode], localePost)

     }
     // localePost.push(postsdetails[0]?.langcode);
     // localePost = _.union([postsdetails[0]?.langcode], localePost)
     
     // for(const val of Object.values(localVar)){
     //     finalLocale = val.code
     // }
     // let testLocale = finalLocale.split("-")[0];

     // var folderpath = entriesFolderPath + '/' + key;
     if (!fs.existsSync(folderpath)) {
         
         mkdirp.sync(folderpath);
         // for(const locale of localePost){
             // dummyLocale = localeSwitch(locale)
             // if(testLocale === localeCode.drupalLocale){
             //     helper.writeFile(path.join(folderpath, `${locale}.json`))                
             // }
         // }
     }
     // if(!fs.existsSync(path.join(folderpath, `${localeCode.csLocale}.json`))){
     //     if(localeCode.csLocale !== undefined && localeCode.csLocale !== null){
             // helper.writeFile(path.join(folderpath, `${localeCode.csLocale}.json`))                    
     //     }
     // }

     // var contenttype = helper.readFile(path.join(folderpath, `${localeCode.csLocale}.json`));
     var contenttype = {};
     return when.promise(function (resolve, reject) {
         ...
         let taxonomyId = helper.readFile(
             path.join(process.cwd(), `drupalMigrationData/entries/taxonomy/${finalLocale}.json`)
         );
         for(const localValue of Object.values(postsdetails)){
             dummyLocale3.push(localValue.langcode);
         }
         for(const localValue of Object.values(postsdetails)){
             dummyLocale3 = _.union([localValue?.langcode], dummyLocale3)
         }
         self.connection.query(contentTypeQuery, function (error, rows, fields) {
             for (var i = 0; i < rows.length; i++) {
                 ...
                 for (const [Fieldkey, data] of Object.entries(postsdetails)) {
                     ...
                     for (var key in field_name) {
                         ...
                         if (field_name[key] == "created") {
                             ...
                             dummyLocale = localeSwitch(data[field_name[key]])
                             if(data[field_name[key]] == dummyLocale.drupalLocale){
                                 ct_value.locale = dummyLocale.csLocale
                             }
                 ...
                 // for(const locale of Object.values(postsdetails)){
                 //     localeCode = localeSwitch(locale.langcode)
                     
                 //     if(locale.langcode === localeCode.drupalLocale){
                 //         // helper.writeFile(path.join(folderpath, `${localeCode.csLocale}.json`), JSON.stringify(contenttype, null, 4))
                 //     }
             ...
             //     if(locale.langcode === localeCode.drupalLocale){
             //         for(const localeVal of Object.values(contenttype)){
             //             if(localeVal.locale === localeCode.csLocale){
             //                 // helper.writeFile(path.join(folderpath, `${localeCode.csLocale}.json`), JSON.stringify(contenttype, null, 4))
             //             }
             //         }
             //     }
             ...
             //     localeCode = localeSwitch(data.langcode);

             //     for(const localeVal of Object.values(contenttype)){
             //         // dummy[localeVal.uid] = localeVal
             //         // if(fs.existsSync(path.join(folderpath, `${localeVal.locale}.json`))){
             //         if(localeVal.locale === localeCode.csLocale){
             //                 // dummy.push(localeVal)
             //             // tempfile = helper.readFile(path.join(path.join(folderpath, `${localeVal.locale}.json`)));
             //             // if(!tempfile.includes(localeVal)){
             //                 fs.writeFileSync(path.join(folderpath, `${localeVal.locale}.json`),
             //                 JSON.stringify(localeVal,null,4),
             //                 {'flag': 'a'}
             //                 )
             //             // }
             //                 // fs.writeFileSync(path.join(folderpath, `${localeVal.locale}.json`),
             ...
             for(const localeVal of Object.values(contenttype)){
                 dummy[localeVal.uid] = localeVal
                 if(fs.existsSync(path.join(folderpath, `${localeVal.locale}.json`))){
                     // if(localeVal.locale === localeVal.locale){
                         // dummy.push(localeVal)
                         fs.writeFileSync(path.join(folderpath, `${localeVal.locale}.json`),
                         JSON.stringify(dummy,null,4),
                         {'flag': 'a'}
                         ) 
                     // }
                 }else{
                     // if(localeVal.locale === localeVal.locale){
                         helper.writeFile(path.join(folderpath, `${localeVal.locale}.json`),
                         JSON.stringify(dummy,null,4)
                         ) 
                     // }
                     // tempfile = helper.readFile(path.join(path.join(folderpath, `${localeVal.locale}.json`)));
