...
Extractfield.prototype = {
 ...
 putfield: function (field) {
     ...
     return when.promise(function (resolve, reject) {
         ...
         var ct = Object.keys(_.keyBy(field, "content_types")).filter(function( element ) {
             return element !== "undefined";
          });
         // var ct = Object.keys(_.keyBy(field, "content_types")).filter(key => key);
         // ct = ct.filter(function( element ) {
         //     return element !== undefined;
         //  });
         ct.map(function (data, index) {
             ...
             when.all(queries).then(function (result) {
                 ...
                 last.unshift("node")
                 result.unshift("SELECT node.nid,node.title,node.langcode,node.created, node.type")
                 var resultdetail = result.join(",")

                 var type = "'" + data + "'";
                 var querydata = resultdetail.concat(" FROM  node_field_data node " + left + " WHERE node.type = " + type)
                 var localeData = `SELECT node.nid,node.langcode, node.type FROM node_field_data node ${left} WHERE node.type = ${type}`
                 countPage = ("SELECT count(node.nid) as countentry FROM  node_field_data node " + left + " WHERE node.type = " + type)
                 select[data] = querydata.toString().replace(/,,/g, ",").replace(/, ,/g, ",");
                 countQuery[data + "Count"] = countPage.toString();
                 localeQuery[data] = localeData.toString();
                 var main = {
                     page: select,
                     count: countQuery,
                     locale: localeQuery
     ...
     return when.promise(function (resolve, reject) {
         ...
         self.connection.query(query, function (error, rows, fields) {
             ...
             for (var i = 0; i < rows.length; i++) {
                 ...
                 details_data.push({ field_name: conv_details.field_name, content_types: conv_details.bundle, type: conv_details.field_type, content_handler: conv_details?.settings?.handler })
